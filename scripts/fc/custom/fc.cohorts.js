(function ($) {
	$.fc.widget("fc.cohorts", $.fc.reportpanel, {
		options: {
			api: {
				getData: "/Analytics.rest/GetCohortReportData",
				
				getReports: "/Misc.rest/GetReports?reportType=cohort",
				saveReport: "/Misc.rest/SaveReport?reportType=cohort",
				removeReport: "/Misc.rest/RemoveReport?reportType=cohort",
			...
			template: 
				...
				<tbody>\
					...
					<tr>\
						<th class="fc-cohorts-title fc-cohorts-header-2 fc-cohorts-row-<%=(i + 1)%>"><%=options.data[i].title%></th>\
						<th class="fc-cohorts-total fc-cohorts-header-2 fc-cohorts-row-<%=(i + 1)%>"><%=$.fc.format(options.data[i].total)%></th>\
						<% var cellValue; %>\
						<% for (var j = 1; j <= options.maxDifference + 1; j++) { %>\
							<% cellValue = _getCellValue(i, options.data[i].diffs[j]); %>\
							<td class="fc-cohorts-value fc-cohorts-row-<%=(i + 1)%> fc-cohorts-col-<%=j%>"><div <%=_getCellStyle(cellValue, options.data[i].total)%>><%=_getFormattedValue(cellValue, options.data[i].total)%></div></td>\
						<% } %>\
					</tr>\
					<% } %>\
				</tbody>',
