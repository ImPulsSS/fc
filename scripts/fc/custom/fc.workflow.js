	...
	$.fc.widget("fc.workflow", {
		...
		_create: function () {
			...
			$("<button>test</button>")
				...
				.click(function () {
					$.fc.dialog.prompt("New block", "Block text", function (value) {
						self.addBlock({
							text: value
						});
					})
		...
		addBlock: function (setup) {
			var block = new $.fc.workflow.block($.extend(true, {
					css: {
						top: this.lastBlock ? this.lastBlock.element.position().top + 150 : 0
					}
				}, setup));
			...
			if (this.lastBlock) {
				block.connectWith(this.lastBlock, { from: "bottom", to: "top" });
			}
